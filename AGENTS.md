...
- Default transport is **stdio**. For Docker ↔ model: `MCP_TRANSPORT=http`, `MCP_AUTH_TOKEN=<secret>`, `READ_ONLY=true`, and if bind is `0.0.0.0` then `MCP_ALLOWED_HOSTS=<service-name>` (required). Stateful sessions: after `initialize` pass `mcp-session-id`. Endpoint `POST|GET|DELETE /mcp`, health `GET /health`.
